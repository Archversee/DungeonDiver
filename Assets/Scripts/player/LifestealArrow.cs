...
public class LifestealArrow : MonoBehaviour
{
    ...
    void OnTriggerEnter2D(Collider2D other)
    {
        if (other.CompareTag("Enemy"))
        {
            ...
            else
            {
                ...
                player.GetComponent<Health>().Heal(dmg);
                Transform healPopupTransform = Instantiate(DamagePopup, player.transform.position, Quaternion.identity);
                DamagePopup healPopup = healPopupTransform.GetComponent<DamagePopup>();
                healPopup.HealSetup(dmg);
                //Destroy(gameObject);
            }
        }
